...
const Movies = ({val}) => {
    ...
    useEffect(() => {
        const getData = async () => {
            let data = await fetch('https://api.themoviedb.org/3/movie/popular?api_key=26a641edc0aa11677d70993667df884e&language=en-US&page=3');
  ...
  return (
    <div className='container'>
        {val && <h4 className='mt-3' style={{color:"purple"}}>Results for <strong>{val}</strong></h4>}
        <div className="row">
            {movie?.map((e) => {
